...
var web3;
var web3etho;
web3 = new Web3(new Web3.providers.HttpProvider('https://rpc.aheios.org'));
web3etho = new Web3(new Web3.providers.HttpProvider('https://rpc.ethoprotocol.com'));
...
//});
exports.athSynced = function(currency) {
    switch(currency) {
        case "ATH":
            res=web3.eth.isSyncing()
                .then(console.log)
            return(res);
            break;
        case "ETHO":
...
exports.athIsAddress = function(currency, address) {
    switch(currency) {
        case "ATH":
            res=web3.utils.isAddress(address)
            return(res);
            break;
        case "ETHO":
            res=web3etho.utils.isAddress(address)
            return(res);
        default:
            logger.error('"#server.ath.athIsAdrdees: Unsupported currency');
    ...
    return new Promise( ( resolve, reject ) => {
        ...
        switch (currency) {
            ...
            case "ETHO":
                ...
                break;
            default:
                logger.error('"#server.ath.athGetBalance: Unsupported currency: %s', currency);
                reject("Unsupported currency");
                break;
...
exports.athdoTransfer= function(currency, fromaddress, depositaddr, depositamount, cb) {
    var rows;
    var wb3;
    
    return new Promise(async (resolve, reject) => {
        logger.info("#server.ath.athdoTransfer: Start process for %s", currency);
        switch (currency) {
            case "ATH":
                wb3 = web3;
                break;
        ...
        logger.info("#server.ath.athdoTransfer: depositamount %s", depositamount);
        // Now let's check if we have enough money
        exports.athGetBalance(currency, fromaddress)
          .then(async function (amount) {
              var BN_amount = wb3.utils.toWei(amount.toString());
              logger.info("#server.ath.athdoTransfer: Amount: %s, Withdraw (incl fee) %s", BN_amount.toString(), BN_depositamountwithfee.toString());
              if (BN_depositamountwithfee.lt(BN_amount)) {
                  logger.error("#server.ath.athdoTransfer: Balance issue: Amount: %s, Withdraw (incl fee) %s", BN_amount.toString(), BN_depositamountwithfee.toString());
                  reject("Not anough money to trigger the transfer. Check Your account");
              } else {
                  // We have, so let's unlock the account
                  // Unlock account for data
    
                  const nonce = await wb3.eth.getTransactionCount(fromaddress, 'latest'); // nonce starts counting from 0
    
                  const transaction = {
                      'to': depositaddr, // faucet address to return eth
                      'value': BN_depositamount,
                      'gas': 30000,
                      'nonce': nonce,
                      // optional data field to send message or execute smart contract
                  };
    
                  const signedTx = await wb3.eth.accounts.signTransaction(transaction, config.ATHPASS);
          ...
          .catch(function (error) {
              logger.error("#server.ath.athdoTrasnfer: Error: Unlock unsuccessful: %s", fromaddress);
              reject ("Can't unlock account: " + error);
          })
    });
